...
async function onRequestPost(context2) {
  try {
    ...
    const ADMIN_PASSWORD = context2.env.ADMIN_PASSWORD || "PCEAdmin2026";
    const ADMIN_TOKEN = context2.env.ADMIN_TOKEN || "pce_secure_token_2026";
...
// api/gallery.js
function isAuthenticated(request, env2) {
  const token = env2.ADMIN_TOKEN || "pce_secure_token_2026";
...
async function onRequestGet(context2) {
  try {
    ...
    if (isHighlight === "1") {
      ...
      const { results: results2 } = await context2.env.DB.prepare(query).all();
    ...
    if (serviceId) {
      ...
      const { results: results2 } = await context2.env.DB.prepare(query).bind(...params).all();
      return new Response(JSON.stringify(results2), { headers: { "Content-Type": "application/json" } });
    }
...
async function onRequestPost2(context2) {
  if (!isAuthenticated(context2.request, context2.env)) {
  ...
  try {
    ...
    if (results.length > 0 && results[0].image_url) {
      try {
        ...
        if (objectKey) {
          await context2.env.BUCKET.delete(objectKey);
        }
      } catch (err) {
        console.error("Failed to delete image from R2:", err);
      }
    }
    await context2.env.DB.prepare("DELETE FROM gallery_photos WHERE id = ?").bind(id).run();
    return new Response(JSON.stringify({ success: true }));
  } catch (error3) {
    return new Response(JSON.stringify({ error: error3.message }), { status: 500 });
  }
...
// api/inquiries.js
function isAuthenticated2(request, env2) {
  const token = env2.ADMIN_TOKEN || "pce_secure_token_2026";
  return request.headers.get("Authorization") === `Bearer ${token}`;
}
__name(isAuthenticated2, "isAuthenticated");
async function onRequestGet2(context2) {
  if (!isAuthenticated2(context2.request, context2.env)) {
    return new Response("Unauthorized", { status: 401 });
  }
  try {
    const { results } = await context2.env.DB.prepare(
...
async function onRequestDelete2(context2) {
  ...
  try {
    const url = new URL(context2.request.url);
    const id = url.searchParams.get("id");
    await context2.env.DB.prepare("DELETE FROM inquiries WHERE id = ?").bind(id).run();
    return new Response(JSON.stringify({ success: true }));
  } catch (error3) {
    return new Response(JSON.stringify({ error: error3.message }), { status: 500 });
  }
...
// api/services.js
function isAuthenticated3(request, env2) {
  const token = env2.ADMIN_TOKEN || "pce_secure_token_2026";
  return request.headers.get("Authorization") === `Bearer ${token}`;
}
__name(isAuthenticated3, "isAuthenticated");
async function onRequestGet3(context2) {
  try {
    const url = new URL(context2.request.url);
    const serviceId = url.searchParams.get("id");
    if (serviceId) {
      const { results: results2 } = await context2.env.DB.prepare(
        "SELECT * FROM services WHERE id = ?"
      ).bind(serviceId).all();
      return new Response(JSON.stringify(results2[0] || null), {
        headers: { "Content-Type": "application/json" }
      });
    }
    const { results } = await context2.env.DB.prepare(
...
async function onRequestPut2(context2) {
  if (!isAuthenticated3(context2.request, context2.env)) {
    return new Response("Unauthorized", { status: 401 });
  }
  try {
    const data = await context2.request.json();
...
// api/upload.js
function isAuthenticated4(request, env2) {
  const token = env2.ADMIN_TOKEN || "pce_secure_token_2026";
  return request.headers.get("Authorization") === `Bearer ${token}`;
}
__name(isAuthenticated4, "isAuthenticated");
async function onRequestPost4(context2) {
  if (!isAuthenticated4(context2.request, context2.env)) {
    return new Response("Unauthorized", { status: 401 });
  }
  try {
    ...
    await context2.env.BUCKET.put(objectKey, file.stream(), {
      httpMetadata: { contentType: file.type || "image/jpeg" }
    });
    const r2Domain = context2.env.R2_PUBLIC_DOMAIN || "https://media.pearlcrownevents.in";
...
// ../.wrangler/tmp/pages-UVMdxb/functionsRoutes-0.9270537075857677.mjs
var routes = [
  {
    routePath: "/api/auth",
    mountPath: "/api",
